...
def get_files(log_folder ='C:/Users/dlab/Downloads/AutoTrainerModular/New folder/Brat', extension = '.dat'):
...
def merge_files_to_df(files):
    ...
    df = pd.concat(map(partial(pd.read_csv, header=None, names = names), files), ignore_index = True).dropna().astype(int)
    return df
...
def trializer_v2(df, stateMachines, trialStartMarker, trialHitMarker, trialMissMarker, sessionStartMarker): # ,trialProbMarker):
    ...
    print(' no of trials in SMs = ', lateSessNPtimes.shape, '\n', 
      'no of rewards in SMs = ', lateSessRewtimes.shape, '\n',
      'no of misses in SMs = ', lateSessMisstimes.shape, '\n',
      'rewards + misses = ', lateSessRewtimes.shape[0]+lateSessMisstimes.shape[0], '\n')
    ...
    lateSessdf['port'] = lateSess[lateSess[0] == trialStartMarker][1].values #what other marker can be used for trial choice? 20 and 21, but those are over-reported. 
...
def trializer_v3(df, list_sessionMarker,
             ...
             rewardProbMarker1, rewardProbMarker2, animal, arms = 2):
    ...
    list_events = [trialStartMarker, trialHitMarker, trialMissMarker, sessionStartMarker,
                   rewardProbMarker1, rewardProbMarker2]
    ...
    indices = np.where((filtered_events[:, 0] == trialStartMarker) | (filtered_events[:, 0]==sessionStartMarker) | (filtered_events[:, 0]==rewardProbMarker1)|(filtered_events[:, 0]==rewardProbMarker2))[0] #indices of events
    ...
    for ind in indices:
        ...
        # append to sess_list
        sess_list.append([npcounter, trialstart, port, reward, trialend, sessnum, datetime, task, event, rewprobl, rewprobl2])
        npcounter += 1
    ...
    sessdf =  pd.DataFrame(sess_list, columns = ['trial#', 'trialstart',
                                   ...
                                   'session#', 'eptime', 'task', 'event', 'rewprobfull1', 'rewprobfull2'])
    
    
    # remove all lines corresponding to start of a session or reward prob marker
    sessdf = sessdf[(sessdf.event!=sessionStartMarker) & (sessdf.event!=rewardProbMarker1)&(sessdf.event!=rewardProbMarker2)].reset_index(drop = True).drop(columns = 'event')
...
def trializer_v3_ind(df, list_sessionMarker,
                 trialStartMarker, trialHitMarker,
                 trialMissMarker, sessionStartMarker,
             rewardProbMarker1, rewardProbMarker2, animal, arms = 2):
    ...
    # filter events as np array to increase speed and avoid repetitive indexing
    filtered_events = df[(df['sm'].isin(list_sessionMarker)) & (df['event'].isin(list_events))].to_numpy()
    indices = np.where((filtered_events[:, 0] == trialStartMarker) | (filtered_events[:, 0]==sessionStartMarker) | (filtered_events[:, 0]==rewardProbMarker1)|(filtered_events[:, 0]==rewardProbMarker2))[0]
    ...
    for ind in indices:
        ...
        if (filtered_events[ind, 0] == rewardProbMarker1) & (filtered_events[ind, 1] == 1):
            rewprobl = filtered_events[ind, 4]
            
        if (filtered_events[ind, 0] == rewardProbMarker2) & (filtered_events[ind, 1] == 2):
            rewprobl2 = filtered_events[ind, 4]
...
def trializer_v3_test(df, list_sessionMarker,
                 trialStartMarker, trialHitMarker,
                 trialMissMarker, sessionStartMarker,
             rewardProbMarker1, rewardProbMarker2, nosePokeMarker1, nosePokeMarker2, animal, arms = 2):
    ...
    list_events = [trialStartMarker, trialHitMarker, trialMissMarker, sessionStartMarker,
                   rewardProbMarker1, rewardProbMarker2, nosePokeMarker1, nosePokeMarker2]
    ...
    for ind in indices:
        ...
        try:
            # find the index of trialMissMarker or trialHitMarker in nextEvents
            status_indices_ports = np.where((nextEvents[:,0]==trialStartMarker) & (nextEvents[:,1]==1))[0]
            # print(status_indices_ports)
            status_index_ports = status_indices_ports[0] if status_indices_ports.size > 0 else None
            status_indices = np.where((nextEvents[:, 0] == trialMissMarker) | (nextEvents[:, 0] == trialHitMarker))[0]
            statusIndex = status_indices[0] if status_indices.size > 0 else None
...
def trializer_v3_test_reversePorts(df, list_sessionMarker,
                 trialStartMarker, trialHitMarker,
                 trialMissMarker, sessionStartMarker,
             rewardProbMarker1, rewardProbMarker2, nosePokeMarker1, nosePokeMarker2, animal, arms = 2):
...
def rew_prob_extractor_v2(df, arms, probMarker1, probMarker2):
    ...
    for ind, prob in enumerate(df[(df['event']==probMarker1) & (df['value'] == 1)]['smtime']):
        temp = prob
        Prob1[session_count] = temp
        session_count += 1
        
    
    for ind, prob in enumerate(df[(df['event']==probMarker2) & (df['value'] == 2)]['smtime']):
        temp = prob
...
def rr_grid(sessdf, n_rows, max_rew, y_min=0.5, y_max=0.85 ): # series of reward-rate plots with increasing session lengths
    l = 50 ##
    ...
    for row in range(n_rows):
        for column in range (2):
            ...
            mov_avg1 = pd.DataFrame(add_reward_vectors(data1)).rolling(30, center = True).mean() 
            axs[row][column].plot(range(len(mov_avg1[0].to_list())), mov_avg1[0].to_list(), 'b')
            axs[row][column].plot(range(len(mov_avg1[0].to_list())), np.ones(len(mov_avg1[0].to_list()))*max_rew, 'k')
...
def choice_unstr(sessdf, trial_cutoff, n_plots): ## trial-by-trial choice plot for unstructured task
    ...
    colors = ["" ,"#f1ee8e", "#f5c77e", "#f69697", "#c30010", "#d24e01", "#e69b00", "#008631" ]
...
def combined_regret(sessdfList, animalList, exclude_sessions, min_length, regret_window, xlim, draw_subset, hard_soft, title):
    ...
    for e in sessdfList:
        ...
        e_subset = e[(e['session#']>exclude_sessions)].groupby('session#').mean(numeric_only = True)
        ...
        for i, row in e_subset.iterrows():
             if (allowed_probs == set([p1[i], p2[i]])) | (allowed_probs2 == set([p1[i], p2[i]]))|(allowed_probs3 == set([p1[i], p2[i]])) |(allowed_probs4 == set([p1[i], p2[i]])):
                keep.append(i)        
        ...
        data_subset = df_subset.groupby("session#").filter(lambda x: len(x["trial#"])>min_length)
        # print(data_subset)
        if hard_soft == 'soft':         
            mov_avg_subset_regret = pd.DataFrame(regret(data_subset, min_length, xlim)).rolling(regret_window, center = True).mean() 
        elif hard_soft == 'hard':
            mov_avg_subset_regret = pd.DataFrame(hard_regret(
                data_subset, min_length, xlim)).rolling(regret_window, center = True).mean()
...
def choice_str(sessdf, trial_cutoff, n_plots): ## trial-by-trial chocie plot for structured task
    #use reward prob as hue
    #in sessdf, add reward prob, with each entry corresponding to the port selected
    sessdf = sessdf.groupby("session#").filter(lambda x: len(x["trial#"])>trial_cutoff)

    colors = ["#cce7c9" ,"#f1ee8e", "#f5c77e", "#f69697", "#c30010", "#d24e01", "#e69b00", "#008631" ]
    # green, yellow, orange, red (light and dark)
    ...
    for i in range(n_plots):
        ...
        markers = {0: "$o$", 1: "o"}
        # hue_order = [10, 20, 30, 40, 60, 70, 80, 90]
        g = sns.relplot(data = session_subset, x = 'trial#', y = 'port', aspect = 5, linewidth = 0.1, hue = hue, style = 'reward', markers = markers, s = 150, legend = 'full', palette = sns.color_palette(colors, 8))
        last = session_subset.groupby('session#')['trial#'].max()
        axes = g.axes.flatten()
        for ax in axes: 
            for ind, l in enumerate(last):
